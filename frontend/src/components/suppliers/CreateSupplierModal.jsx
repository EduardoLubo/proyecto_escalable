...
const CreateSupplierModal = ({ show, token, setSuccessMessage, setErrorMessage, onClose, onCreated, onError }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const { data: response } = await axios.post(`${BASE_URL}api/v1/proveedor`, formData, {
                headers: { Authorization: `Bearer ${token}` },
            });
            setFormData({
                codigo: "",
            ...
            setSuccessMessage(response.message || "Proveedor creado exitosamente.");
            onCreated();
        } catch (error) {
