...
import DetailCrewModal from "./DetailCrewModal";
...
const Crews = () => {
    ...
    const [selectedDetailCrew, setSelectedDetailCrew] = useState(null);
    ...
    const fetchCrews = async (currentPage = 1) => {
        try {
            ...
            const { data } = await axios.get(`${BASE_URL}api/v1/cuadrilla?${queryParams}`, {
                headers: {
    ...
    const handleExportExcel = (data) => {
        ...
        const exportData = data.map((item) => ({
            ...
            Código: item.codigo || "S/D",
            Descripción: item.descripcion || "S/D",            
            Jefe: item.cuadrilla_personal.find(p => p.rol === "JEFE DE CUADRILLA")?.personal_cuadrilla?.nombre || "S/D",
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Cuadrillas");

        // Generar el archivo Excel y descargarlo
        const excelBuffer = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
        const blob = new Blob([excelBuffer], { type: "application/octet-stream" });
        saveAs(blob, `Cuadrillas_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };
    ...
    const columns = [
        ...
        {
            header: "Jefe de Cuadrilla",
            size: 380,
            Cell: ({ row }) => {
                const crew = row.original;
                const jefe = crew.cuadrilla_personal.find(p => p.rol === "JEFE DE CUADRILLA")?.personal_cuadrilla?.nombre || "S/D"
                return jefe;
            },
        },
        {
    ...
    const table = useMaterialReactTable({
        ...
        muiTableBodyCellProps: ({ cell, table }) => ({
            onClick: () => {
                ...
                setSelectedDetailCrew(cell.row.original);
        ...
        <div className="rounded-xl dark:shadow-dark-md shadow-md bg-white dark:bg-darkgray p-6 relative w-full break-words">
            ...
            <DeleteCrewModal
                ...
                onDeleted={async () => {
                    ...
                    const newPage = (crews.length === 1 && page > 1) ? page - 1 : page;
                    setPage(newPage);
                    await fetchCrews(newPage);
                }}
                onError={() => {
                    setIsModalDeleteOpen(false);
                    setCrewToDelete(null);
            ...
            <DetailCrewModal
                ...
                onEdit={() => {
                    setIsModalDetailOpen(false);
                    setCrewToEdit(selectedDetailCrew);
                    setIsModalEditOpen(true);
                }}
            />
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl">Cuadrillas</h2>
