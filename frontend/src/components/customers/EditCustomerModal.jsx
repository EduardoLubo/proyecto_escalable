...
const EditCustomerModal = ({ show, onClose, customer, token, onUpdated, onError, setSuccessMessage, setErrorMessage }) => {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const { data: response } = await axios.put(`${BASE_URL}api/v1/cliente/${customer.cliente_id}`, formData, {
                headers: {
    ...
    const hasChanges = () => {
        return (
            formData.codigo !== customer.codigo ||
            formData.descripcion !== customer.descripcion ||
            formData.activo !== customer.activo ||
            formData.operativo !== customer.operativo
        );
    };
            ...
            <div className="p-4">
                ...
                <ModalBody className="max-h-[75vh] overflow-y-auto">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
                        ...
                        <div>
                            <Label htmlFor="operativoEdit" value="Operativo PSM" className="mb-2 block" />
                            <Select
                                id="operativoEdit"
                                name="operativo"
                                value={formData.operativo ? "true" : "false"}
                                className="select-md"
                                onChange={(e) => setFormData({ ...formData, operativo: e.target.value === "true" })}
                            >
