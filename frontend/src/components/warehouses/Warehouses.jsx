...
import DetailWarehouseModal from "./DetailWarehouseModal";
...
const Warehouses = () => {
    ...
    const [selectedDetailWarehouse, setSelectedDetailWarehouse] = useState(null);
    ...
    const fetchWarehouses = async (currentPage = 1) => {
        try {
            ...
            const { data } = await axios.get(`${BASE_URL}api/v1/deposito?${queryParams}`, {
                headers: {
    ...
    const handleExportExcel = (data) => {
        ...
        saveAs(blob, `Depositos_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };
    ...
    const table = useMaterialReactTable({
        ...
        muiTableBodyCellProps: ({ cell, table }) => ({
            onClick: () => {
                ...
                setSelectedDetailWarehouse(cell.row.original);
        ...
        renderRowActionMenuItems: ({ row, table, closeMenu }) => [
            <MRT_ActionMenuItem
                ...
                onClick={() => {
                    closeMenu?.();
                    setWarehouseToEdit(row.original);
        ...
        <div className="rounded-xl dark:shadow-dark-md shadow-md bg-white dark:bg-darkgray p-6 relative w-full break-words">
            ...
            <DeleteWarehouseModal
                ...
                onDeleted={async () => {
                    ...
                    const newPage = (warehouses.length === 1 && page > 1) ? page - 1 : page;
                    setPage(newPage);
                    await fetchWarehouses(newPage);
                }}
                onError={() => {
                    setIsModalDeleteOpen(false);
                    setWarehouseToDelete(null);
            ...
            <DetailWarehouseModal
                ...
                onEdit={() => {
                    setIsModalDetailOpen(false);
                    setWarehouseToEdit(selectedDetailWarehouse);
                    setIsModalEditOpen(true);
                }}
            />
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl">Depósitos</h2>
