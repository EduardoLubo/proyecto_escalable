...
export const handleOpenPDF = async ({ userData, stocks, getUbicacionDescripcionFromCache, getCuadrillaDescripcionFromCache, formatFechaHora, codigoToPdf }) => {
    ...
    pdf.text(`CONTROL`, colRight, 18, { align: "right" });
    ...
    const tableBody = stocks.map((p) => {
        ...
        const tipoMov = p.movimiento?.tipo_movimiento?.descripcion;

        // --- Ubicación ---
        const verOrigenUbicacion = tipoMov === "CONSUMO EN OBRA";
        const ubicacion = verOrigenUbicacion ? p.movimiento?.desde_ubicacion : p.movimiento?.hacia_ubicacion;
        ...
        const cuadrillaId = verOrigenCuadrilla ? p.movimiento?.desde_cuadrilla_id : p.movimiento?.hacia_cuadrilla_id;
        const cuadrillaDesc = getCuadrillaDescripcionFromCache(cuadrillaId) || "S/D";

        return [
            formatFechaHora(p.movimiento?.auditoria_alta) || "S/D",
            p.movimiento?.usuario?.legajo || "S/D",
            p.movimiento?.cliente?.codigo || "S/D",
            tipoMov || "S/D",
            ubicacionDesc,
            cuadrillaDesc,
            p.movimiento_detalle_serial?.material?.serie || "S/D",
            p.cantidad || "S/D",
            p.movimiento?.movimiento_id || "S/D",
        ];
    });

    autoTable(pdf, {
        startY: tableStartY,
        head: [["Fecha", "Usuario", "Cliente", "Movimiento", "Ubicación", "Cuadrilla", "Serie", "Cantidad", "Remito"]],
        body: tableBody,
        headStyles: {
        ...
        columnStyles: {
            7: { halign: 'right' }, // Remito
            8: { halign: 'right' }, // Cantidad
        },
